...
- **Discover now scans reviews automatically, in the same run.** Search a profession+city and it finds the businesses AND scans every one of their reviews for negatives, all in one workflow run — no separate manual "Scan all reviews for negatives" step needed anymore. The Discover tab shows the negative reviews as soon as the search finishes. This does mean a discover search now takes noticeably longer (workflow timeout bumped to 300 min to match).
